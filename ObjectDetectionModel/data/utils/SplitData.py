...
for filename in tqdm(os.listdir(imgpath_WilderPerson)):
    if filename.endswith(postfix):
        ...
        label_path = os.path.join(labelpath_WilderPerson, filename[:-len(postfix)] + 'txt')
        all_images.append(image_path)
        all_labels.append(label_path)
...
train_images, val_test_images, train_labels, val_test_labels = train_test_split(all_images, all_labels, test_size=val_size+test_size, random_state=0)
val_images, test_images, val_labels, test_labels = train_test_split(val_test_images, val_test_labels, test_size=test_size, random_state=0)
