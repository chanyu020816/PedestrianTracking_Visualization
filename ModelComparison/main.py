...
class VideoTracker(Detector):
    def __init__(
        ...
        self.video_output_path: str = (
            os.path.join(
                "./Output",
                f"{model}_{self.run_time}_thold{self.detection_threshold}.mp4",
            )
            if self.save
            else None
        )
...
class ImageDetection(Detector):
    def __init__(
        ...
        self.image_output_path: str = (
            os.path.join(
                "./Output",
                f"{model}_{self.run_time}_thold{self.detection_threshold}.png",
            )
            if self.save
            else None
        )
...
def main(
    model: str, name: str, save: bool, video: bool, detection_threshold: float
) -> None:
    if video:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type", "-T", type=str, default="video", choices=["image", "video"]
    )
